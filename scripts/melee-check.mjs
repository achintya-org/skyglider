...
const res=await page.evaluate(()=>{ window.__zoo(); return window.__meleeTest(); });
await browser.close();server.close();
console.log("melee:",JSON.stringify(res));
const ok = res && res.after > res.before && errors.length===0;
console.log("checks:",JSON.stringify({killed:!!(res&&res.after>res.before),noErrors:errors.length===0}));
